CREATE OR REPLACE VIEW construction_dw.vw_project_progress_summary AS
SELECT
    p.project_id,
    p.project_name,
    p.project_type,
...
FROM construction_dw.fact_project_progress f
JOIN construction_dw.dim_project p USING (project_key)
JOIN construction_dw.dim_date d USING (date_key)
GROUP BY p.project_id, p.project_name, p.project_type;

CREATE OR REPLACE VIEW construction_dw.vw_budget_vs_actual AS
SELECT
    p.project_id,
    p.project_name,
    p.budget_myr,
...
FROM construction_dw.fact_project_costs f
JOIN construction_dw.dim_project p USING (project_key)
GROUP BY p.project_id, p.project_name, p.budget_myr;

CREATE OR REPLACE VIEW construction_dw.vw_equipment_utilization AS
SELECT
    p.project_name,
    e.equipment_id,
    e.equipment_name,
...
FROM construction_dw.fact_equipment_usage f
JOIN construction_dw.dim_project p USING (project_key)
JOIN construction_dw.dim_equipment e USING (equipment_key)
GROUP BY p.project_name, e.equipment_id, e.equipment_name, e.equipment_type;

CREATE OR REPLACE VIEW construction_dw.vw_material_consumption AS
SELECT
    p.project_name,
    m.material_id,
    m.material_name,
...
FROM construction_dw.fact_project_costs f
JOIN construction_dw.dim_project p USING (project_key)
JOIN construction_dw.dim_material m USING (material_key)
WHERE f.cost_category = 'Material'
GROUP BY p.project_name, m.material_id, m.material_name, m.category, m.unit;

CREATE OR REPLACE VIEW construction_dw.vw_safety_incident_summary AS
SELECT
    p.project_name,
    f.severity,
    ...
    COUNT(*) FILTER (WHERE f.reportable) AS reportable_incidents
FROM construction_dw.fact_safety_incidents f
JOIN construction_dw.dim_project p USING (project_key)
GROUP BY p.project_name, f.severity;

CREATE OR REPLACE VIEW construction_dw.vw_project_delay_risk AS
WITH latest AS (
    SELECT DISTINCT ON (f.project_key)
        f.project_key, f.actual_progress_pct, f.schedule_variance_pct, d.full_date
    FROM construction_dw.fact_project_progress f
    JOIN construction_dw.dim_date d USING (date_key)
    ORDER BY f.project_key, d.full_date DESC
), delay_totals AS (
